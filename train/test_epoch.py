...
for data in test_dataset.take(200):
  psnr, ssim, fid = generate_images(data, generator, training=False, num_epoch=step)
  psnr = df_load['psnr'].values.tolist()
  psrn = CSV_reader(psnr)

  ssim = df_load['ssim'].values.tolist()
  ssim = CSV_reader(ssim)

  fid = df_load['fid'].values.tolist()
  fid = CSV_reader(fid)
