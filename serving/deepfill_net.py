"""DeepFillv2 생성기 (PyTorch) — gated convolution + contextual attention.

원 논문: Yu et al., "Free-Form Image Inpainting with Gated Convolution" (ICCV 2019).
이 네트워크 정의는 아래 PyTorch 포팅을 참고해 벤더링했다. TensorFlow 원본 가중치를
변환한 states_tf_*.pth(cnum_in=5, cnum=48)를 그대로 로드하도록 맞춰져 있다.

  출처: https://github.com/nipponjo/deepfillv2-pytorch (model/networks_tf.py)
  참고: https://github.com/zhaoyuzhi/deepfillv2

시연 목적의 추론 전용으로만 사용한다. 원 구현/가중치의 저작권은 각 저작자에게 있다.
"""
