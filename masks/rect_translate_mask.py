...
def brush_stroke_mask(FLAGS, name='mask', _test_num=[0]):
    """Generate mask tensor from bbox.

    Returns:
        np.array: Mask array of shape [1, H, W, 1]
    """
    import numpy as np

    def generate_mask_square(H, W, T):
        """
        Generate square mask from bounding box.
        ...
        # Adjust square size based on image dimensions
        square_width = W // 7
        square_height = H // 7

        # Draw square
        bottom_right_x = min(top_left_x + square_width, W)
        bottom_right_y = min(top_left_y + square_height, H)
    ...
    img_shape = FLAGS.img_shapes
    height = img_shape[0]  # Image height
    width = img_shape[1]  # Image width

    # Generate mask
    mask = generate_mask_square(height, width, _test_num[0])
    _test_num[0] += 1  # Increment test number for next call
